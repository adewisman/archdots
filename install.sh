...
configure_installation() {
    ...
    # Use mapfile to read lsblk output into an array
    ...
    while true; do
        ...
        if [[ $choice =~ ^[0-9]+$ ]] && [ "$choice" -ge 1 ] && [ "$choice" -le "${#disks[@]}" ]; then
            # Extract just the device name (e.g., sda, nvme0n1)
    ...
    # --- Other Variables ---
    echo -e "\n${YELLOW}2. Enter the rest of the configuration values (press Enter to accept defaults).${NC}"
    ...
    echo -e "The following settings will be used. This will ${RED}DESTROY ALL DATA${NC} on the selected disk."
    echo "----------------------------------------------------"
...
# Call the interactive setup function
configure_installation
...
echo ">>> Adding the ArchZFS repository..."
...
echo ">>> Importing and signing the ArchZFS PGP key..."
pacman-key --init &>/dev/null
pacman-key --recv-keys 3A9917BF0DED5C13F69AC68FABEC0A1208037BE9 &>/dev/null
...
sgdisk -n1:1M:+512M -t1:EF00 "$DISK" &>/dev/null  # EFI System Partition
sgdisk -n2:0:0 -t2:BF00 "$DISK" &>/dev/null       # ZFS Partition
sleep 2
EFI_PART="${DISK}1"
# Handle nvme naming convention (p1, p2) vs sata/scsi (1, 2)
if [[ $DISK == *"nvme"* ]]; then
    EFI_PART="${DISK}p1"
    ZFS_PART="${DISK}p2"
else
...
pacstrap -K /mnt base base-devel linux linux-firmware linux-headers zfs-dkms limine efibootmgr nano networkmanager curl git &>/dev/null
