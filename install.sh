...
configure_installation() {
    ...
    if [ ${#disks[@]} -eq 0 ]; then
        echo -e "${RED}No disks found. Aborting.${NC}"; exit 1;
    fi
    for i in "${!disks[@]}"; do
        printf "  %s) %s\n" "$((i+1))" "${disks[$i]}";
    done
    ...
    while true; do
        ...
        if [[ $choice =~ ^[0-9]+$ ]] && [ "$choice" -ge 1 ] && [ "$choice" -le "${#disks[@]}" ]; then
            ...
            DISK="/dev/${DISK_NAME}"
            echo -e "${GREEN}You have selected: ${DISK}${NC}"; break
        else
    ...
    read -p "Enter hostname [archzfs]: " HOSTNAME; HOSTNAME=${HOSTNAME:-archzfs}
    ...
    if [[ "${confirm,,}" != "y" ]]; then
        echo -e "${YELLOW}Installation aborted by user.${NC}"; exit 0;
    fi
}
...
pacstrap -K /mnt base base-devel linux linux-firmware linux-headers zfs-dkms limine efibootmgr nano networkmanager curl git sudo &>/dev/null
