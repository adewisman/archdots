...
echo -e "  1. Adding the official Xlibe binary repository."
echo -e "  2. ${RED}REMOVING your standard Xorg server packages.${NC}"
echo -e "  3. Installing the Xlibe display server and a full dev/gaming environment."
echo -e "  4. Installing 'aurify' and configuring Snapper with pacman hooks and limits."
echo -e "  5. Enabling automated weekly BTRFS scrubbing."
...
systemctl enable btrfs-scrub@-.timer # <-- NEW: Enable weekly scrub for the root filesystem

echo -e "${GREEN}--> Configuring Snapper with pacman hook and snapshot limits...${NC}"
snapper -c root create-config /

# Modify the new config file to set limits
...
# =================================================================
echo -e "${GREEN}--> Installing and theming Limine bootloader...${NC}"
limine-install
convert -size 1920x1080 xc:"#24283b" -font "JetBrains-Mono-Nerd-Font-Complete-Mono" -pointsize 48 -fill "#a9b1d6" -gravity center -draw "text 0,0 'Arch Linux'" /boot/limine-bg.png
ROOT_UUID=$(findmnt -n -o UUID /)
mkdir -p /boot/limine
cat << EOF > /boot/limine.cfg
TIMEOUT=3
TERM_MODE=max
TERM_WALLPAPER=boot:///limine-bg.png
...
# =================================================================
echo -e "${GREEN}--> Configuring and theming Ly display manager...${NC}"
LY_CONFIG_DIR="/etc/ly"
mkdir -p ${LY_CONFIG_DIR}
convert -size 1920x1080 xc:"#1a1b26" /usr/share/ly-bg.png
...
echo -e "Automated weekly ${YELLOW}BTRFS file system scrubbing${NC} is now enabled."
